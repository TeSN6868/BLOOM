...
import 'dart:convert';
import 'bloom_messaging_service.dart';
...
class BloomApi {
  ...
  static Future<Map<String, dynamic>> updateProfileStatus({
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(
        'profile_status_http_${response.statusCode}: ${response.body}',
      );
    }
    ...
    if (data is! Map || data['ok'] != true) {
      throw Exception('profile_status_update_failed');
    }
...
class _BloomStoryViewerState extends State<_BloomStoryViewer> {
  ...
  Widget _buildImage() {
    return Image.network(
      ...
      errorBuilder: (_, _, _) {
        return _textStory(fallback: 'Foto Story tidak dapat ditampilkan.');
      },
    );
  }
  ...
  Widget _textStory({String? fallback}) {
    final content = widget.text.isNotEmpty
        ? widget.text
        : (fallback ?? 'BLOOM');
...
class BloomVoiceRecorder {
  ...
  static Future<Map<String, dynamic>> updateProfileStatus({
    ...
    if (response.statusCode != 200) {
      throw Exception('profile_status_http_${response.statusCode}');
    }

    final data = jsonDecode(response.body);

    if (data is! Map || data['ok'] != true) {
      throw Exception('profile_status_invalid_response');
    }
...
// ============================================================
// BLOOM MESSAGING UI V1
// ============================================================

class BloomMessagesPage extends StatefulWidget {
  const BloomMessagesPage({super.key});

  @override
  State<BloomMessagesPage> createState() => _BloomMessagesPageState();
}

class _BloomMessagesPageState extends State<BloomMessagesPage> {
  late final BloomMessagingService _messaging;

  List<BloomConversation> _conversations = const [];
  bool _loading = true;
  String? _error;
  String? _userId;
  ...
  void initState() {
    ...
    _messaging = const BloomMessagingService(
      baseUrl: BloomApi.baseUrl,
    );
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final conversations = await _messaging.getConversations(
        userId: userId,
      );
  ...
  Future<void> _openConversation(
    BloomConversation conversation,
  ) async {
        ...
        actions: [
          IconButton(
            onPressed: () {
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (_) => const BloomStartChatPage(),
                ),
              );
            },
            tooltip: 'Pesan Baru',
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          final displayName =
              conversation.otherName.trim().isNotEmpty
                  ? conversation.otherName.trim()
...
class BloomStartChatPage extends StatefulWidget {
  const BloomStartChatPage({super.key});

  @override
  State<BloomStartChatPage> createState() => _BloomStartChatPageState();
}

class _BloomStartChatPageState extends State<BloomStartChatPage> {
  late final BloomMessagingService _messaging;
  ...
  Future<void> _start() async {
    ...
    final otherUserId = _userIdController.text.trim();

    if (otherUserId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Masukkan ID pengguna BLOOM.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (userId == otherUserId) {
        throw Exception('Tidak bisa mengirim pesan ke diri sendiri');
      }
      ...
      final conversation = await _messaging.createConversation(
        userId: userId,
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              'Masukkan ID pengguna BLOOM yang ingin kamu ajak bicara.',
              style: TextStyle(
                color: softText,
                fontWeight: FontWeight.w600,
              ),
...
class BloomChatPage extends StatefulWidget {
  final String userId;
  ...
  @override
  State<BloomChatPage> createState() => _BloomChatPageState();
}

class _BloomChatPageState extends State<BloomChatPage> {
  late final BloomMessagingService _messaging;
  late final TextEditingController _controller;
  late final ScrollController _scrollController;

  List<BloomMessage> _messages = const [];
  bool _loading = true;
  bool _sending = false;
  String? _error;
  ...
  Widget build(BuildContext context) {
    final displayName =
        widget.conversation.otherName.trim().isNotEmpty
            ? widget.conversation.otherName.trim()
            : widget.conversation.otherUsername.trim().isNotEmpty
                ? '@${widget.conversation.otherUsername.trim()}'
                : 'BLOOM User';
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(
          children: [
            _MessagingAvatar(
              photoUrl: widget.conversation.otherPhotoUrl,
              name: displayName,
              radius: 20,
            ),
            const SizedBox(width: 10),
      ...
      itemBuilder: (context, index) {
        ...
        return Align(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: mine ? premiumBlue : Colors.white,
...
class _MessagingAvatar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CircleAvatar(
      ...
      backgroundColor: premiumBlue,
      backgroundImage:
          resolved == null ? null : NetworkImage(resolved),
      child: resolved == null
          ? Text(
